...
v(fd,ed);fd.prototype.toString=function(){return this.h};new fd("about:blank",Uc);new fd("about:invalid#zTSz",Uc);function gd(a){hd();var b=rb();a=b?b.createScriptURL(a):a;return new sb(a,tb)}
...
function wm(a){var b=a.csn;a=a.root_ve_type;if(b&&a&&(ak(b,a),a=Wj()))for(var c in tm){var d=tm[c];d&&kk(b,a,d)}}
...
window.location.href;-1!=d.indexOf("?")?(d=(d||"").split("#")[0],d=d.split("?",2),d=dh(1<d.length?d[1]:d[0])):d={};d.forced_experiments&&(c.serializedForcedExperimentIds=d.forced_experiments)}Z=Lk(a,c.WEB_PLAYER_CONTEXT_CONFIG_ID_EMBEDDED_PLAYER,!1)}else Z=Lk(a);Z.addEventListener("onScreenChanged",wm);Z.addEventListener("onLogClientVeCreated",um);Z.addEventListener("onLogServerVeCreated",xm);Z.addEventListener("onLogVeClicked",ym);Z.addEventListener("onLogVesShown",zm);Z.addEventListener("onVideoDataChange",
vm);a=N("POST_MESSAGE_ID","player");N("ENABLE_JS_API")?sm=new cl(Z):N("ENABLE_POST_API")&&"string"===typeof a&&"string"===typeof b&&(rm=new dl(window.parent,a,b),sm=new Yk(Z,rm.j));Fj()},void 0);
